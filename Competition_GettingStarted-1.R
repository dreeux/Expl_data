...
SimpleMod = glm(NYTimesBlogTrain$Popular ~. , data=NewsTrain, family=binomial)

remov = select(NYTimesBlogTrain, -Popular)
new = rbind(NYTimesBlogTest, remov)
new$Headline = as.character(new$Headline); new$Abstract = as.character(new$Abstract); new$Snippet = as.character(new$Snippet)
NewsTrain = new[1:nrow(NYTimesBlogTrain), ];NewsTest = new[6533:8402,]

SimpleMod = glm(NYTimesBlogTrain$Popular ~. , data=NewsTrain, family=binomial)

NewsTrain = cbind(NewsTrain, NYTimesBlogTrain$Popular)
